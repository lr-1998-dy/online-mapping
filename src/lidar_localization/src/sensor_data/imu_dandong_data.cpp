...
bool lidar_localization::IMUDandongData::origin_position_inited = false;
GeographicLib::LocalCartesian lidar_localization::IMUDandongData::geo_converter;
int lidar_localization::IMUDandongData::sensor_check = 0;

namespace lidar_localization {
void IMUDandongData::InitOriginPosition(double latitude,double longitude,double altitude) {
    geo_converter.Reset(latitude, longitude, altitude);
    origin_position_inited = true;
}

void IMUDandongData::GetENUPosition(){
...
Eigen::Matrix3f IMUDandongData::GetOrientationMatrix() {
    Eigen::Matrix3f matrix = orientation.matrix().cast<float>();
    return matrix;
}

bool IMUDandongData::SyncData(std::deque<IMUDandongData>& UnsyncedData, std::deque<IMUDandongData>& SyncedData, double sync_time) {
    ...
    IMUDandongData front_data = UnsyncedData.at(0);
    IMUDandongData back_data = UnsyncedData.at(1);
    IMUDandongData synced_data;
