...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    if (registration_method == "NDT-OMP") {
        registration_ptr = std::make_shared<NDTOmpRegistration>(config_node[registration_method]);
        return true;
    } 

    if (registration_method == "NDT-IRLS") {
        registration_ptr = std::make_shared<NDTIrlsRegistration>(config_node[registration_method]);
        return true;
    } 
...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    std::cout<<"ScanMatch  Time:   "<<std::setprecision(9)<<(ros::Time::now()-t0)<<std::endl;
    cloud_pose = current_frame_.pose;
...
/**
 * @description:将init_pose_设置为单位矩阵
 * @param {Matrix4f&} init_pose
 * @return {*}
 */
...
bool FrontEnd::SetGNSSInitPose(const Eigen::Matrix4f& init_pose) {
    init_gnss_pose_ = init_pose;
    return true;
}
