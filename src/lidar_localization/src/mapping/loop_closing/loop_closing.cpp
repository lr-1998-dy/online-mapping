...
bool LoopClosing::Update(const KeyFrame key_frame, const KeyFrame key_gnss) {
    ...
    //lr:找到距离我最近的位姿，记录他的index
    //lr:是通过GNSS的位姿保持一致的
    //因为GNSS和lidar_odom的index都是一致的，相当于是对齐过的
...
//lr:闭环检测的代码，是通过GNSS查找的距离当前关键帧最近的一帧数据（也是GNSS的数据）
...
//lr:主要找到之前相近的位姿，构建子地图，然后进行匹配得到一个位姿（观测得到的）
//lr:整个过程是在gnss坐标系下面实现的，但是为什么在加入后端的时候是用的AddSe3Edge而不是AddSe3PriorXYZEdge,不是很理解
bool LoopClosing::CloudRegistration(int key_frame_index) {
    ...
    // 匹配
    //lr:其中的local_map是由gnss求得的，而scan_map则是在雷达自身坐标系下面的
    //lr:这样也符合Registration的要求
