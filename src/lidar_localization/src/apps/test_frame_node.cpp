...
int main(int argc, char *argv[]) {
    ...
    std::shared_ptr<CloudSubscriber> cloud_sub_ptr = std::make_shared<CloudSubscriber>(nh, "/synced_cloud", 100000);
    std::shared_ptr<OdometrySubscriber> gnss_sub_ptr = std::make_shared<OdometrySubscriber>(nh, "/synced_gnss", 1000000);
   std::shared_ptr<IMUSubscriber> imu_sub_ptr = std::make_shared<IMUSubscriber>(nh, "/kitti/oxts/imu", 1000000);
    ...
    std::deque<PoseData> gnss_data_buff;
    ...
    while (ros::ok()) {
        ...
        while (cloud_data_buff.size() > 0 &&  gnss_data_buff.size() > 0) {
            CloudData cloud_data = cloud_data_buff.front();
            PoseData gnss_data = gnss_data_buff.front();

            double d_time = cloud_data.time - gnss_data.time;
            if (d_time < -0.05) {
                ...
                odometry_matrix=gnss_data.pose;
